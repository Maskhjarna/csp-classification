...
const auto test_index_to_function_input = TestSingle{
	...
	test_eq(cspc::__internal::index_to_function_input(44, 4, 3), {1, 1, 2, 2}),
};

const auto test_function_input_to_index = TestSingle{
	"function input to index",
	test_eq(cspc::__internal::function_input_to_index({1, 1, 2, 2}, 3), DomainValue(44)),
};

const auto test_satisfies_identity_rare = TestBundle{
	"satisfies identity rare",
	{
		test_eq(cspc::__internal::satisfies_identity({0, 0, 0, 0}, {0, 1, 0, 2}), true),
		test_eq(cspc::__internal::satisfies_identity({0, 1, 0, 1}, {0, 1, 0, 2}), true),
		test_eq(cspc::__internal::satisfies_identity({0, 0, 0, 5}, {0, 1, 0, 2}), true),
		test_eq(cspc::__internal::satisfies_identity({5, 2, 5, 3}, {0, 1, 0, 2}), true),
		test_eq(cspc::__internal::satisfies_identity({0, 0, 1, 0}, {0, 1, 0, 2}), false),
		test_eq(cspc::__internal::satisfies_identity({0, 1, 1, 0}, {0, 1, 0, 2}), false),
		test_eq(cspc::__internal::satisfies_identity({0, 1, 1, 1}, {0, 1, 0, 2}), false),
		test_eq(cspc::__internal::satisfies_identity({5, 1, 2, 3}, {0, 1, 0, 2}), false),
	},
};
